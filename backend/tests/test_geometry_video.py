...
def main():
    ...
    logger.add(logs_dir / "test_geometry_video.log", rotation="500 MB")
    
    # 读取示例数据
    json_path = root_dir / "data/samples/math_problems/sample_math_problem_003.json"
    ...
    # 检查公式内容并打印出来
    for step in data['blackboard']['steps']:
        for element in step['elements']:
            if element['type'] == 'formula':
                logger.info(f"检测到公式元素: {element['content']}")
                # 将公式更新为简单的可渲染公式
                element['content'] = "$OA=5,\\quad OD=3$"
    ...
    # 设置输出路径
    output_path = output_dir / "test_geometry_video.mp4"
